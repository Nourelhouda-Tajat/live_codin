...
function getAvailableDestinations() {
    ...
    for (let i = 0; i < spaceData.destinations.length; i++) {
        if (spaceData.destinations[i].available===true) {
            availableDestination.push(spaceData.destinations[i])
        }
...
function calculateTotalRevenue() {
    ...
    for (let i = 0; i < spaceData.bookings.length; i++) {
        somme+=spaceData.bookings[i].totalPrice;   
    }
    return somme;
}
...
function findUserByEmail(email) {
    for (let i = 0; i < spaceData.users.length; i++) {
        if(email===spaceData.users[i].email){
...
function countTotalPassengers() {
    let cont=0;
   for (let i = 0; i< spaceData.bookings.length; i++) {
    cont+=spaceData.bookings[i].passengers.length;
    }
    return cont;
}
...
function findMostExpensiveBooking() {
    let max=spaceData.bookings[0];
    for (let i = 1; i < spaceData.bookings.length; i++) {
            
            if (max.totalPrice<=spaceData.bookings[i].totalPrice) {
...
function updateBookingStatus(bookingId, newStatus) {
    for (let i = 0; i < spaceData.bookings.length; i++) {
        if (bookingId===spaceData.bookings[i].id) {
            spaceData.bookings[i].status=newStatus;
        }
        
    }
    return spaceData.bookings[bookingId];
